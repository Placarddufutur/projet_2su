...
public class ClockInAppletClass extends Applet implements ClockInAppletInterface {
	...
	private static final short SW_WRONG_HOUR = (short) 0x6403;
	...
	public boolean clockInRequest(APDU apdu) {
		...
		if ((dataIn[1] > (byte)7 && dataIn[1] < (byte)12) || ((dataIn[1] > (byte)13 && dataIn[1] < (byte)18))) {
			...
			ISOException.throwIt(SW_WRONG_HOUR);
			return false;
		}
	...
	private void getClockInLogs(APDU apdu) {
		...
		apdu.setOutgoingLength((byte)clockInLog.length);
		
		
		for (short i = (short)0; i < (short) clockInLog.length; i ++) {
			buffer[i] = clockInLog[i];
		}

		apdu.sendBytes((short)0, (short)clockInLog.length);
	}
}
