...
public class AccessControllerAppletClass extends Applet implements AccessControllerAppletInterface {
	...
	final static byte GENERATE_SECRET_MASTER_INS = (byte) 0x31;
	...
	private static byte[] master_secretTab;
	private static byte[] master_hashTab;
	private static byte[] master_xorTab;
	...
	final static short SW_WRONG_PASSWORD = (short) 0x6301;
	final static short SW_ID_NOT_FOUND = (short) 0x6302;
	final static short SW_UNAUTHORIZED_ACCESS = (short) 0x6303;
	...
	public void process(APDU apdu) throws ISOException {
		...
		case GENERATE_SECRET_INS:
			...
			break;
		case GENERATE_SECRET_MASTER_INS:
			if (MASTER_PIN_AUTHENTICATED)
				generateMasterSecret(apdu);
			else
				ISOException.throwIt(SW_VERIFICATION_FAILED);
			break;
...
//			ISOException.throwIt(SW_WRONG_PASSWORD);
	...
	public boolean verifyMasterPassword(APDU apdu) {
		if (MASTER_PIN_AUTHENTICATED) {
	...
	//		messageDigest.doFinal(resultXOR, (short) 0, (short) (resultXOR.length), hashedSecret, (short) 0);
			...
			if (Util.arrayCompare(resultXOR, (short) 0, master_xorTab, (short) 0, (short) (master_xorTab.length)) == (byte) 0) {
	//		if (Util.arrayCompare(hashedSecret, (short) 0, hashTab, (short) 0, (short) (hashTab.length)) == (byte) 0) {
	//		 hash are the same, user successfully auth w/ password
				successfullyAuth = true;
	...
	public void generateMasterSecret(APDU apdu) {
		// Formule : Hash_SHA1(random_Int ^ mdp_user)
		// Générer nombre aléatoire pour un utilisateur qui sera stocké, et passer au
		// XOR le
		// nombre aléatoire avec le mdp à hasher, puis hasher le tout en SHA-1
		...
		master_secretTab = new byte[random.length];
		master_hashTab = new byte[hashedSecret.length];
		master_xorTab = new byte[resultXOR.length];
		Util.arrayCopyNonAtomic(random, (short) 0, master_secretTab, (short) 0, (short) random.length);
		Util.arrayCopyNonAtomic(hashedSecret, (short) 0, master_hashTab, (short) 0, (short) hashedSecret.length);
		Util.arrayCopyNonAtomic(resultXOR, (short) 0, master_xorTab, (short) 0, (short) resultXOR.length);

		short le = apdu.setOutgoing();

		if (master_hashTab.length < le) {
			le = (short) master_hashTab.length;
		}

		apdu.setOutgoingLength(le);
		apdu.sendBytesLong(master_hashTab, (short) 0, le);
		MASTER_PIN_AUTHENTICATED = false;
	}
